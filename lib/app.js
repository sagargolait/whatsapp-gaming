...
app.post('/webhook', async (req, res) => {
  ...
  try {
    if (user.mode === 'single-player') {
      console.log('Handling single-player mode request');
      await singlePlayerModeHandler(req, res);
      console.log('Single-player mode handler completed');
    } else if (user.mode === 'multi-player') {
      console.log('Handling multi-player mode request');
      await multiPlayerModeHandler(req, res);
      console.log('Multi-player mode handler completed');
    } else {
      const userSession = {
        phone: req.body.From,
        mode: 'single-player'
